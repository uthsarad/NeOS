...
## 2026-06-25 - Redundant Repository Checkout in CI
**Learning:** In GitHub Actions workflows, jobs that solely utilize API-driven tools like the `gh` CLI (e.g., `gh pr merge "$PR_URL"`) to interact with issues or pull requests do not require a local checkout of the repository's codebase. The `actions/checkout` step inherently clones the repository, incurring unnecessary network transfer, storage allocation, and execution time for workflows that operate entirely via HTTP API endpoints.
**Action:** When creating or optimizing CI/CD workflows, explicitly omit the `actions/checkout` step if the job executes purely API-based tasks (like PR management or issue labeling) without needing local repository context, saving runner resources and accelerating execution.
