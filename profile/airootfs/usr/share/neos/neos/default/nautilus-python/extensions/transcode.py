...
class TranscodeAction(GObject.GObject, Nautilus.MenuProvider):
    def _launch_transcode(self, paths):
        wrapper = shutil.which("neos-launch-floating-terminal-with-presentation")
        binary = shutil.which("neos-transcode")
    ...
    def _make_item(self, paths):
        ...
        item = Nautilus.MenuItem(
            name="NeOSTranscodeNautilus::transcode",
            label=label,
    ...
    def _tools_available(self):
        return bool(
            shutil.which("neos-launch-floating-terminal-with-presentation")
            and shutil.which("neos-transcode")
        )
