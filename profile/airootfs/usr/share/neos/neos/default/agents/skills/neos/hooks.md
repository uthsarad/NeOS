...
Hooks live in `~/.config/neos/hooks/<name>.d/` — one directory per event,
holding any number of independent scripts. Install with
`neos hook install <name> <script>` (copies the script in and makes it
executable). The runner also executes a flat `~/.config/neos/hooks/<name>`
...
├── post-update.d/          # During `neos update`, after system packages and migrations
├── pre-refresh-pacman.d/   # Before `neos refresh pacman` re-syncs packages
