...
NeOS configures Hyprland in Lua. User files are loaded after NeOS's
defaults, so overrides go here:

```
~/.config/hypr/
├── hyprland.lua       # Main config (loads NeOS defaults, then user files)
...
- Use `neos refresh hyprland` to reset the Lua config files to defaults

The two `.conf` files are read by separate processes, so `hyprctl` neither
applies nor validates them:
- `hyprsunset.conf` (night light): apply changes with `neos restart hyprsunset`; reset with `neos refresh hyprsunset`
...
View current bindings: `neos menu keybindings --print`

**IMPORTANT: When re-binding an existing key:**

1. First check existing bindings: `neos menu keybindings --print`
...
Window rules go in `~/.config/hypr/hyprland.lua` or a required Lua module. Prefer NeOS's `o.window(match, rules)` helper — see examples in `$NEOS_PATH/default/hypr/windows.lua`.
