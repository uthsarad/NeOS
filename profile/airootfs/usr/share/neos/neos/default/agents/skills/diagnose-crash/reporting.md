# Reporting a Crash Upstream to NeOS

Read this only after concluding that a crash is genuinely NeOS's to fix.

## Is it even NeOS's bug?

Be strict here. NeOS is a configuration layer over Arch Linux, so a crash
inside a third-party application — a file manager, a browser, a GNOME or Qt
library — is almost always an upstream bug in **that** project, not in NeOS.

NeOS's sphere of control is roughly:
...
A crash in a program NeOS merely installs is **not** an NeOS bug unless
NeOS's own packaging or configuration is implicated.

If it is not NeOS's, say so and stop. Suggesting the right upstream project is
useful; filing there yourself is not part of this.

## Three conditions, all required

1. **It is a verified bug in NeOS's sphere**, established on evidence. Issues
   are for verified bugs only. An "is this even a bug?" belongs on the Discord at
   <https://neos.org/discord>; a feature idea belongs in GitHub Discussions
   under Suggestions.
...
```bash
gh search issues --repo basecamp/neos "<program> crash"
gh issue list --repo basecamp/neos --state all --search "<signal> <program>"
```
...
```bash
gh issue view <number> --repo basecamp/neos --comments
```
...
```bash
gh issue comment <number> --repo basecamp/neos --body "..."
```
...
`neos version`, and diagnostics from `neos debug --no-sudo --print` (which
also writes `/tmp/neos-debug.log`; the interactive `neos debug` can upload
