...
Phase ordering (full-disk and protected/pre-mounted):
    ...
    arch_install_system    → one archinstall flow for partition/mount-or-use,
                             base install, early NeOS packages, Limine setup,
                             useradd, runtime NeOS packages, fstab
    configure_hibernation  → root-owned swap/resume drop-ins
    run_system_finalizer   → arch-chroot root neos-apply-system, including Snapper
    finalize_limine_boot   → final Limine config/UKI build after hardware drop-ins
    run_chroot_finalizer   → arch-chroot -u user neos-provision-user
...
# names explicitly instead of relying on provides=neos resolution.
def _iso_ref() -> str:
    if ref := os.environ.get("NEOS_ISO_REF"):
        return ref.strip()

    ref_file = Path("/root/neos_iso_ref")
    if ref_file.exists():
        try:
...
def _default_package_targets() -> dict[str, str]:
    if _iso_ref() in {"dev", "local"}:
        return {
            "runtime": "neos-dev",
            "settings": "neos-settings-dev",
            "nvim": "neos-nvim",
        }

    return {
        "runtime": "neos",
        "settings": "neos-settings",
        "nvim": "neos-nvim",
    }
...
def _package_targets() -> dict[str, str]:
    ...
    targets_file = Path("/usr/share/neos-iso/package-targets")
    if targets_file.exists():
        try:
            for raw in targets_file.read_text().splitlines():
                ...
                match key.strip():
                    case "NEOS_RUNTIME_PACKAGE":
                        targets["runtime"] = value
                    case "NEOS_SETTINGS_PACKAGE":
                        targets["settings"] = value
                    case "NEOS_NVIM_PACKAGE":
                        targets["nvim"] = value
        except OSError:
            pass

    env_to_key = {
        "NEOS_RUNTIME_PACKAGE": "runtime",
        "NEOS_SETTINGS_PACKAGE": "settings",
        "NEOS_NVIM_PACKAGE": "nvim",
    }
...
def _neos_runtime_package() -> str:
    return _package_targets()["runtime"]


def _neos_settings_package() -> str:
    return _package_targets()["settings"]


def _neos_nvim_package() -> str:
    return _package_targets()["nvim"]


# Packages installed BEFORE useradd. The selected neos-settings package and
# neos-nvim populate /etc/skel so the user's home gets seeded correctly, and
# neos-settings also ships the limine/snapper configs. Target-side setup
# commands are installed later by the selected NeOS runtime package and
# executed in chroot.
...
# Install LuaRocks before neos-nvim pulls in lua51-lpeg. Arch's lua-luarocks
...
def _early_bootstrap_packages() -> list[str]:
    return [*EARLY_BOOTSTRAP_BASE_PACKAGES, _neos_settings_package()]


def _early_user_seed_packages() -> list[str]:
    return [_neos_nvim_package()]
...
#
# archinstall is patched in the wrapper (neos-iso-install) BEFORE Python
...
def prepare_live(ctx: InstallContext) -> None:
    if ctx.is_protected:
        ...
        if disk:
            ...
            subprocess.run(["neos-iso-cleanup-disk", disk], check=True)
...
# Reorders guided.py's perform_installation() so early NeOS packages install
# before user creation and before our NeOS-owned Limine setup copies files
...
def arch_install_system(ctx: InstallContext) -> None:
    ...
    a pre-mounted target, and NeOS derives boot/fstab details from that same
    input.
    """
    ...
    with arch.open_installer(config, ctx.target, silent=True) as installer:
        ...
        try:
            ...
            info("› installing NeOS runtime + neos-base.packages")
...
def _configure_limine_boot(ctx: InstallContext, installer, config) -> None:
    ...
    if not arch.is_limine(config):
        raise RuntimeError("NeOS installs only support Limine bootloader setup")

    info("› installing bootloader (Limine)")
    if arch.is_pre_mount(config):
        _install_pre_mounted_limine(ctx)
    else:
        _install_limine_neos(ctx, installer, config)
...
def _install_limine_neos(ctx: InstallContext, installer, config) -> None:
...
def _install_pre_mounted_limine(ctx: InstallContext) -> None:
    ...
    if not esp_device:
        raise RuntimeError("neos_install.storage.esp_device missing")
...
def _write_limine_pacman_hook(target: Path, hook_command: str) -> None:
    hook_contents = textwrap.dedent(
        ...
        [Action]
        Description = Deploying NeOS Limine after upgrade...
    ...
    (hooks_dir / "99-neos-limine.hook").write_text(hook_contents)
...
def _limine_template(ctx: InstallContext, filename: str) -> Path:
    candidates = [
        ctx.target / "usr" / "share" / "neos" / "install" / "assets" / "limine" / filename,
        ctx.target / "usr" / "share" / "neos" / "default" / "limine" / filename,
        ctx.neos_path / "install" / "assets" / "limine" / filename,
        ctx.neos_path / "default" / "limine" / filename,
    ]
...
# installed kernel. neos-apply-system's hardware scripts routinely install
...
def _drop_archinstall_zram_conf(ctx: InstallContext) -> None:
    ...
    neos-settings ships the tuning as a vendor drop-in at
    /usr/lib/systemd/zram-generator.conf.d/90-neos.conf, which outranks the
...
def _install_early_packages(installer) -> None:
    ...
    info(f"› installing early NeOS packages: {', '.join(bootstrap_packages)}")
...
def _mount_offline_package_cache(ctx: InstallContext) -> None:
    ...
    """
    source = Path("/var/cache/neos/mirror/offline")
...
def _mask_mkinitcpio_pacman_hooks(
    ...
    for name in names:
        path = hooks_dir / name
        backup = hooks_dir / f"{name}.neos-backup"
...
def _runtime_package_list(ctx: InstallContext) -> list[str]:
    """Selected NeOS runtime package + every package in the ISO-bundled
    base package list that isn't already installed early."""
    base_pkgs_file = Path("/usr/share/neos-iso/neos-base.packages")
    pkgs = [_neos_runtime_package()]
    already_installed = set(_early_packages()) | {
        _neos_runtime_package(),
        _neos_settings_package(),
        _neos_nvim_package(),
        "neos",
        "neos-settings",
        "neos-nvim",
...
# ─────────────────────────────────────────────────────────────────────────────
# Install intent helpers: normalize the NeOS-specific part of the
...
def _boot_intent(ctx: InstallContext) -> dict:
    boot = dict(ctx.neos_install.get("boot") or {})
...
def _storage_intent(ctx: InstallContext) -> dict:
    return dict(ctx.neos_install.get("storage") or {})
...
def verify_protected_mounts(ctx: InstallContext) -> None:
    ...
    for key in ("esp_device", "root_device"):
        ...
        if not device:
            raise RuntimeError(f"protected mode: neos_install.storage.{key} missing")
    ...
    info(f"› protected target verified: kernel={storage.get('kernel', 'linux-neos')} esp={boot['esp_mount']}")
...
def _btrfs_root_device(ctx: InstallContext) -> str:
    ...
    if storage.get("luks_uuid"):
        return storage.get("root_mapper") or "/dev/mapper/neos_root"
...
def _write_pre_mounted_fstab(ctx: InstallContext) -> None:
    ...
    lines = [
        "# /etc/fstab — generated by NeOS ISO",
...
def _write_pre_mounted_crypttab(ctx: InstallContext) -> None:
    ...
    crypttab.write_text(f"neos_root  UUID={luks_uuid}  none  luks,discard\n")


def _build_pre_mounted_cmdline(ctx: InstallContext, btrfs_uuid: str) -> str:
    storage = _storage_intent(ctx)
    if storage.get("luks_uuid"):
        root_mapper = storage.get("root_mapper") or "/dev/mapper/neos_root"
        return (
            f"cryptdevice=UUID={storage['luks_uuid']}:neos_root "
...
def configure_hibernation(ctx: InstallContext) -> None:
    ...
    """
    setup = ctx.target / "usr" / "bin" / "neos-hibernation-setup"
    if not setup.exists():
        _debug_log(ctx, "skipping hibernation: /usr/bin/neos-hibernation-setup is not installed")
        return
    ...
    subprocess.run([
        ...
        "NEOS_INSTALL_LOG_FILE=/var/log/neos-install.log",
        "/usr/bin/neos-hibernation-setup", "--force", "--no-rebuild",
    ], check=True)


def _install_debug_enabled() -> bool:
    return os.environ.get("NEOS_INSTALL_DEBUG") == "1" or Path("/usr/share/neos-iso/install-debug").exists()
...
#  3. arch-chroot as root → neos-apply-system --first-install
#  4. arch-chroot as user → neos-provision-user --first-install
# ─────────────────────────────────────────────────────────────────────────────
...
def _prepare_target_setup(ctx: InstallContext) -> None:
    ...
    bind_mounts = [
        ("/var/cache/neos/mirror/offline", "/var/cache/neos/mirror/offline"),
...
def _ensure_finalizer_log_started(ctx: InstallContext) -> tuple[str, int]:
    if "neos_start_time" not in ctx.state:
        ctx.state["neos_start_epoch"] = int(time.time())
        ctx.state["neos_start_time"] = time.strftime("%Y-%m-%d %H:%M:%S")

    ctx.log_path.parent.mkdir(parents=True, exist_ok=True)
    ctx.log_path.touch(exist_ok=True)
    ctx.log_path.chmod(0o666)

    if not ctx.state.get("neos_finalizer_header_written"):
        with ctx.log_path.open("a", encoding="utf-8") as log:
            log.write(f"=== NeOS Target Setup Started: {ctx.state['neos_start_time']} ===\n")
        ctx.state["neos_finalizer_header_written"] = True

    return ctx.state["neos_start_time"], ctx.state["neos_start_epoch"]
...
def _run_target_setup_command(ctx: InstallContext, cmd: list[str], *, user: str | None = None) -> None:
    ...
    neos_start_time, neos_start_epoch = _ensure_finalizer_log_started(ctx)

    target_log = ctx.target / "var" / "log" / "neos-install.log"
    ...
    mirror_channel = _read_neos_mirror()
    env_extras = [
        "NEOS_PATH=/usr/share/neos",
        "NEOS_INSTALL=/usr/share/neos/install",
        f"NEOS_INSTALL_USER={ctx.username}",
        f"NEOS_START_TIME={neos_start_time}",
        f"NEOS_START_EPOCH={neos_start_epoch}",
        f"NEOS_USER_NAME={ctx.full_name}",
        f"NEOS_USER_EMAIL={ctx.email}",
        f"NEOS_MIRROR={mirror_channel}",
        f"NEOS_ISO_REF={_iso_ref()}",
        f"NEOS_RUNTIME_PACKAGE={_neos_runtime_package()}",
        f"NEOS_SETTINGS_PACKAGE={_neos_settings_package()}",
        f"NEOS_NVIM_PACKAGE={_neos_nvim_package()}",
        "NEOS_INSTALL_LOG_FILE=/var/log/neos-install.log",
        "NEOS_LOG_TO_STDOUT=1",
    ]
    if _install_debug_enabled():
        env_extras.append("NEOS_INSTALL_DEBUG=1")
...
def run_system_finalizer(ctx: InstallContext) -> None:
    if ctx.defer_provisioning:
        cmd = ["/usr/bin/neos-apply-system", "--defer-provisioning", "--first-install"]
    else:
        cmd = ["/usr/bin/neos-apply-system", "--install-user", ctx.username, "--first-install"]
...
# setup (neos-provision-owner) and factory reset (neos-system-factory-reset) consume.
#
# Every install stashes the bundled Node tarball in /var/lib/neos/provisioning/
...
PROVISION_STATE_DIR = "var/lib/neos/provisioning"
PROVISION_KEYFILE = "etc/neos/provisioning.key"
...
def stage_provisioning_state(ctx: InstallContext) -> None:
    ...
    service_src = ctx.target / "usr/share/neos/install/provisioning/neos-provision-owner.service"
    setup_bin = ctx.target / "usr/bin/neos-provision-owner"
    if not service_src.exists() or not setup_bin.exists():
        raise RuntimeError(
            "deferred-provisioning install requested, but the installed NeOS runtime does not ship "
            "first-boot setup (neos-provision-owner + install/provisioning/neos-provision-owner.service). "
    ...
    unit_dst = ctx.target / "etc/systemd/system/neos-provision-owner.service"
    ...
    link = wants_dir / "neos-provision-owner.service"
    link.unlink(missing_ok=True)
    link.symlink_to("/etc/systemd/system/neos-provision-owner.service")
...
def _stage_provisioning_luks_unlock(ctx: InstallContext, provisioning_dir) -> None:
    ...
    cmdline_dropin.write_text(
        'KERNEL_CMDLINE[default]+=" cryptkey=rootfs:/etc/neos/provisioning.key"\n'
    )

    files_dropin = ctx.target / "etc/mkinitcpio.conf.d/99-neos-provisioning-key.conf"
    files_dropin.parent.mkdir(parents=True, exist_ok=True)
    files_dropin.write_text("FILES+=(/etc/neos/provisioning.key)\n")
...
def finalize_limine_boot(ctx: InstallContext) -> None:
    ...
    if "NeOS" not in limine_conf.read_text():
        raise RuntimeError(f"{limine_conf} has no NeOS entry")
...
def run_chroot_finalizer(ctx: InstallContext) -> None:
    ...
    _run_target_setup_command(
        ctx,
        ["/usr/bin/neos-provision-user", "--force", "--first-install"],
        user=ctx.username,
    )
...
def _read_neos_mirror() -> str:
    p = Path("/root/neos_mirror")
    return p.read_text().strip() if p.exists() else "stable"


# ─────────────────────────────────────────────────────────────────────────────
# configure_login: seed SDDM's last user/session for the password-only NeOS
...
def configure_login(ctx: InstallContext) -> None:
    ...
    (sddm_dir / "99-neos-login.conf").write_text(
        "[Theme]\nCurrent=neos\n\n"
    ...
    if ctx.encrypt and not ctx.defer_provisioning:
        autologin_conf.write_text(
            "[Autologin]\n"
            f"User={ctx.username}\n"
            "Session=neos.desktop\n"
        )
    else:
        # deferred-provisioning installs have no user yet; neos-provision-owner writes autologin
...
# keys an autoinstall drive supplied. A stock NeOS install ships openssh but
...
def configure_ssh_access(ctx: InstallContext) -> None:
    ...
    if ctx.defer_provisioning:
        ...
        # neos-provision-owner to install once first boot creates the owner, and
...
[Service]
Type=simple
ExecStart=/usr/bin/sh -c 'until tailscale up --auth-key file:{TAILSCALE_AUTHKEY_TARGET}; do sleep 15; done; rm -f {TAILSCALE_AUTHKEY_TARGET}; systemctl disable neos-tailscale-join.service'
...
def configure_tailscale(ctx: InstallContext) -> None:
    ...
    subprocess.run(
        ["arch-chroot", str(ctx.target), "systemctl", "enable",
         "tailscaled.service", "neos-tailscale-join.service"],
        check=True,
    )
...
def validate_boot(ctx: InstallContext) -> None:
    ...
    if "NeOS" not in limine_conf_text:
        raise RuntimeError(f"{limine_conf} has no NeOS entry")
    ...
    uki_prefix = _limine_setting(config_text, "CUSTOM_UKI_NAME", "neos") or "neos"
    kernel = storage.get("kernel") or (ctx.user_configuration.get("kernels") or ["linux-neos"])[0]
    ...
    if arch.has_uefi():
        ...
        # Hardware packages (neos-hw-intel-ptl, …) can swap the kernel out
...
def _validate_provisioning_state(ctx: InstallContext) -> None:
    ...
    link = ctx.target / "etc/systemd/system/multi-user.target.wants/neos-provision-owner.service"
    if not link.is_symlink():
        raise RuntimeError("deferred-provisioning install but neos-provision-owner.service is not enabled")
...
def _assert_boot_hooks_restored(ctx: InstallContext) -> None:
    ...
    for name in TARGET_DEFERRED_BOOT_HOOKS:
        ...
        # limine-mkinitcpio-hook is a hard dependency of the NeOS runtime
...
# Taken at the end of every install, it is what makes neos-system-factory-reset a
# true factory reset.
# ─────────────────────────────────────────────────────────────────────────────
...
FACTORY_SCRUB_PATHS = (
    "var/lib/neos/provisioning/authorized_keys",
    "var/lib/neos/provisioning/luks-key",
    "etc/neos/provisioning.key",
    "etc/limine-entry-tool.d/99-neos-provisioning-unlock.conf",
    "etc/mkinitcpio.conf.d/99-neos-provisioning-key.conf",
    "etc/tailscale/authkey",
    "etc/systemd/system/neos-tailscale-join.service",
    "etc/systemd/system/multi-user.target.wants/neos-tailscale-join.service",
)
...
def cleanup_protected_state(ctx: InstallContext) -> None:
    ...
    if Path("/dev/mapper/neos_root").exists():
        subprocess.run(
            ["cryptsetup", "close", "neos_root"],
            check=False,
            capture_output=True,
        )
