...
NEOS_INSTALL_* environment variables before Python starts. This keeps
archinstall's import-time CLI parsing from seeing NeOS-specific flags.
"""
...
def build_phases(ctx: InstallContext):
    ...
    and the partition details NeOS needs for boot/fstab generation.
    """
    ...
    return [
        ...
        ("Installing Arch + NeOS",  arch_install_system),
...
def main() -> int:
    ...
    info(f"Installing NeOS for {who} → {ctx.target}")
