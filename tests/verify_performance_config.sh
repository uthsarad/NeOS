...
if [[ -f "$CONFIG_FILE" ]]; then
    ...
    if [[ "$CONFIG_CONTENT" =~ btrfs:[^\n]*compress=zstd:1[^\n]*discard=async[^\n]*space_cache=v2 ]]; then
        echo "  [PASS] space_cache=v2 found in main Btrfs mount options"
    else
        echo "[FAIL] space_cache=v2 NOT found in main Btrfs mount options (or not correctly placed)"
        # Show context for debugging
        grep "btrfs:" "$CONFIG_FILE" || true
        exit 1
    fi
else
    echo "[WARN] Calamares config not found ($CONFIG_FILE). Skipping Calamares performance checks."
fi
...
if [[ ! -f "$SYSCTL_FILE" ]]; then
    echo "[FAIL] Missing performance config file: $SYSCTL_FILE"
    exit 1
fi
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')vm\.swappiness[^\n]*=[^\n]*100 ]]; then
    echo "  [PASS] vm.swappiness set to 100 (optimized for ZRAM)"
else
    echo "[FAIL] vm.swappiness is NOT set to 100"
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')vm\.page-cluster[^\n]*=[^\n]*0 ]]; then
    echo "  [PASS] vm.page-cluster set to 0 (optimized for ZRAM)"
else
    echo "[FAIL] vm.page-cluster is NOT set to 0"
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')vm\.vfs_cache_pressure[^\n]*=[^\n]*50 ]]; then
    echo "  [PASS] vm.vfs_cache_pressure set to 50 (optimized for responsiveness)"
else
    echo "[FAIL] vm.vfs_cache_pressure is NOT set to 50"
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')vm\.max_map_count[^\n]*=[^\n]*2147483642 ]]; then
    echo "  [PASS] vm.max_map_count set to 2147483642 (optimized for gaming)"
else
    echo "[FAIL] vm.max_map_count is NOT set to 2147483642"
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')net\.core\.default_qdisc[^\n]*=[^\n]*cake ]]; then
    echo "  [PASS] net.core.default_qdisc set to cake (bufferbloat mitigation)"
else
    echo "[FAIL] net.core.default_qdisc is NOT set to cake"
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')net\.ipv4\.tcp_congestion_control[^\n]*=[^\n]*bbr ]]; then
    echo "  [PASS] net.ipv4.tcp_congestion_control set to bbr (congestion control)"
else
    echo "[FAIL] net.ipv4.tcp_congestion_control is NOT set to bbr"
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')vm\.dirty_ratio[^\n]*=[^\n]*10 ]]; then
    echo "  [PASS] vm.dirty_ratio set to 10 (optimized for I/O latency)"
else
    echo "[FAIL] vm.dirty_ratio is NOT set to 10"
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')vm\.dirty_background_ratio[^\n]*=[^\n]*5 ]]; then
    echo "  [PASS] vm.dirty_background_ratio set to 5 (optimized for I/O latency)"
else
    echo "[FAIL] vm.dirty_background_ratio is NOT set to 5"
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')net\.ipv4\.tcp_fastopen[^\n]*=[^\n]*3 ]]; then
    echo "  [PASS] net.ipv4.tcp_fastopen set to 3"
else
    echo "[FAIL] net.ipv4.tcp_fastopen is NOT set to 3"
...
if [[ "$SYSCTL_CONTENT" =~ (^|$'\n')net\.ipv4\.tcp_slow_start_after_idle[^\n]*=[^\n]*0 ]]; then
    echo "  [PASS] net.ipv4.tcp_slow_start_after_idle set to 0"
else
    echo "[FAIL] net.ipv4.tcp_slow_start_after_idle is NOT set to 0"
...
if [[ ! -f "$MODULES_FILE" ]]; then
    echo "[FAIL] Missing network modules file: $MODULES_FILE"
    exit 1
fi

MODULES_CONTENT=$(<"$MODULES_FILE")
if [[ "$MODULES_CONTENT" =~ (^|$'\n')tcp_bbr ]]; then
    echo "  [PASS] tcp_bbr module enabled"
else
    echo "[FAIL] tcp_bbr module NOT enabled"
    grep "tcp_bbr" "$MODULES_FILE" || true
    exit 1
fi

if [[ "$MODULES_CONTENT" =~ (^|$'\n')sch_cake ]]; then
    echo "  [PASS] sch_cake module enabled"
else
    echo "[FAIL] sch_cake module NOT enabled"
