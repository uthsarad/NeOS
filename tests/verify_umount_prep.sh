...
if [[ ! -f "$CONF" ]]; then
    echo "[FAIL] $CONF missing"; FAIL=1
else
    if grep -qE "^dontChroot:[[:space:]]*true" "$CONF"; then
        echo "  [PASS] unmount-prep.conf runs on the host (dontChroot: true)"
    else
        echo "[FAIL] unmount-prep.conf must set 'dontChroot: true' (needs host mount namespace)"; FAIL=1
    fi
    # Must recursively unmount the target /dev subtree.
    if grep -qE "umount .*-R.*\\\$\{ROOT\}/dev" "$CONF"; then
        echo "[PASS] recursively unmounts \${ROOT}/dev"
    else
        echo "[FAIL] unmount-prep.conf must run a recursive 'umount -R ... \${ROOT}/dev'"; FAIL=1
    fi
fi
...
if grep -qE "id:[[:space:]]*unmountprep" "$SETTINGS" \
   && grep -qE "config:[[:space:]]*unmount-prep.conf" "$SETTINGS"; then
    echo "  [PASS] 'unmountprep' shellprocess instance registered"
else
    echo "[FAIL] settings.conf missing 'unmountprep' instance (id + config: unmount-prep.conf)"; FAIL=1
fi
...
' "$SETTINGS"; then
    echo "  [PASS] exec order: cleanup -> unmountprep -> umount"
else
    echo "[FAIL] exec sequence must be cleanup -> shellprocess@unmountprep -> umount"; FAIL=1
fi
