...
if [ ! -f "tools/neosctl/main.go" ] || [ ! -f "tools/neosctl/go.mod" ]; then
    echo "[FAIL] tools/neosctl files missing!"
    exit 1
fi
...
if ! command -v go &> /dev/null; then
    ...
    if grep -q 'func runAudit' tools/neosctl/main.go; then
        echo "[FAIL] runAudit reappeared in neosctl — profile auditing is owned by tools/neos-profile-audit"
        exit 1
    fi
    echo "  [PASS] Go neosctl source structure verified."
    exit 0
fi
...
grep -q 'func runRankMirrors' tools/neosctl/main.go
echo "[PASS] neosctl compiles and no longer duplicates the profile audit."
...
if /tmp/neosctl rank-mirrors "$MIRRORLIST" > /tmp/neosctl_rank.log 2>&1; then
    echo "  [PASS] neosctl rank-mirrors executed successfully (live network available)."
else
    if grep -q "no active Server URLs found" /tmp/neosctl_rank.log; then
        echo "[FAIL] neosctl failed to parse the profile mirrorlist"
        cat /tmp/neosctl_rank.log
        exit 1
    fi
    echo "[WARN] Live network unavailable for mirror ranking; command dispatch verified."
fi
rm -f /tmp/neosctl /tmp/neosctl_rank.log
echo "[PASS] Go neosctl audit passed successfully."
