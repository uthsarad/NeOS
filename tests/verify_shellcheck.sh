...
if ! command -v shellcheck >/dev/null 2>&1; then
  ...
  find tests/ profile/airootfs/usr/local/bin/ -type f \( -name '*.sh' -o -path 'profile/airootfs/usr/local/bin/*' ! -name '*.*' \) ! -name 'neos-welcome-app' -print0 | xargs -0 -r shellcheck --format=gcc || true
