...
if [ ! -f "tools/neos_tasks.rb" ] || [ ! -f "Rakefile" ]; then
    echo "[FAIL] Ruby tasks or Rakefile missing!"
    exit 1
fi
...
if ! command -v ruby &> /dev/null; then
    ...
    if grep -q 'def audit_profile' tools/neos_tasks.rb; then
        echo "[FAIL] audit_profile reappeared in neos_tasks.rb — profile auditing is owned by tools/neos-profile-audit"
        exit 1
    fi
    echo "  [PASS] Ruby tasks structure verified."
    exit 0
fi

echo "Verifying Ruby task runner (canonical audit: tools/neos-profile-audit)..."
ruby tools/neos_tasks.rb audit .
echo "[PASS] Ruby tasks audit passed successfully."
