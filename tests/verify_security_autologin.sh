...
if grep -qE '^\s*-\s*unpackfs\s*$' "$SETTINGS"; then
    echo "[FAIL] installer still runs unpackfs (live clone)"; FAIL=1
else
...
if grep -qE '^\s*-\s*users\s*$' "$SETTINGS"; then
    echo "  [PASS] Calamares 'users' module is in sequence (prompts for user/password)"
else
    echo "[FAIL] Calamares 'users' module missing (no username/password prompt)"; FAIL=1
fi
...
if grep -q "shellprocess@liveuser" "$SETTINGS"; then
    echo "[FAIL] installer runs shellprocess@liveuser (kiosk mode left over)"; FAIL=1
else
    echo "  [PASS] installer does not run shellprocess@liveuser"
fi

# 4. The live session autologin still uses the unprivileged liveuser (not root).
if grep -q "^User=liveuser$" "$SDDM_AUTOLOGIN"; then
    echo "  [PASS] live SDDM autologin user is liveuser"
else
    echo "[FAIL] live SDDM autologin user is not liveuser"; FAIL=1
fi
