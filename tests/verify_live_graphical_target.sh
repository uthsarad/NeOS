...
if [[ -L "$SDDM_WANT" ]]; then
    ...
    echo "  [PASS] graphical.target.wants/sddm.service -> $target"
else
    echo "[FAIL] $SDDM_WANT missing — live ISO boots to TTY instead of SDDM"; FAIL=1
fi
...
if [[ -L "$DEFAULT" ]]; then
    ...
    if [[ "$target" == *"graphical.target" ]]; then
        echo "  [PASS] default.target -> $target"
    else
        echo "[FAIL] default.target points to '$target', must be graphical.target"; FAIL=1
    fi
else
    echo "[FAIL] $DEFAULT missing — systemd falls back to compiled-in default (may vary)"; FAIL=1
fi
...
if grep -q "User=liveuser" "$AUTOLOGIN" && grep -q "Session=plasma" "$AUTOLOGIN"; then
    echo "[PASS] SDDM autologin configured for liveuser / plasma"
else
    echo "[FAIL] $AUTOLOGIN must set User=liveuser and Session=plasma"; FAIL=1
fi
...
if [[ -f "$CUSTOMIZE" ]] && grep -q "useradd" "$CUSTOMIZE" && grep -q "liveuser" "$CUSTOMIZE"; then
    echo "[PASS] customize_airootfs.sh present and creates liveuser at build time"
else
    echo "[FAIL] $CUSTOMIZE missing or does not create liveuser — live session will be unreliable"; FAIL=1
fi
...
if grep -q "Before=.*sddm\.service" "$SERVICE"; then
    echo "[PASS] neos-liveuser-setup.service runs Before=sddm.service"
else
    echo "[FAIL] neos-liveuser-setup.service missing Before=sddm.service"; FAIL=1
fi
