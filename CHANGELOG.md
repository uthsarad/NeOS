...
- **Pull requests now run the gate suite**: `build-iso.yml` gained a `pull_request` trigger for `testing`/`main`, with the `build` job guarded by `if: github.event_name != 'pull_request'` so the release/publish path (GitHub Release, SourceForge push, up to 90 minutes) never runs from a PR. The `test` job previously ran only on pushes to `testing` and on manual dispatch, so the gates it enforces could not gate a change before it landed.
